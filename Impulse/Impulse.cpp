...
Impulse::Impulse() : CubeApplication(40) {
...
Impulse::Drop::Drop(Vector3i maxPos, Vector3f pos, Vector3f vel, Vector3f accel, Color col)
...
bool Impulse::Drop::getRdyDelete() {
    return rdyDelete_;
}


Impulse::Particle::Particle(Vector3f pos, Vector3f vel, Vector3f accel, Color col)
        : position_(pos),
          velocity_(vel),
          acceleration_(accel),
          color_(col) {}

void Impulse::Particle::step() {
    accelerate();
    move();
}

void Impulse::Particle::move() {
    position_ += velocity_;
}

void Impulse::Particle::accelerate() {
    velocity_ += acceleration_;
}

Vector3f Impulse::Particle::position() {
    return position_;
}

Vector3f Impulse::Particle::velocity() {
    return velocity_;
}

Vector3f Impulse::Particle::acceleration() {
    return acceleration_;
}

Vector3i Impulse::Particle::iPosition() {
    return Vector3i(round(position()[0]), round(position()[1]), round(position()[2]));
}

Vector3i Impulse::Particle::iVelocity() {
    return Vector3i(round(velocity()[0]), round(velocity()[1]), round(position()[2]));
}

Vector3i Impulse::Particle::iAcceleration() {
    return Vector3i(round(acceleration()[0]), round(acceleration()[1]), round(acceleration()[2]));
}

void Impulse::Particle::position(Vector3f pos) {
    position_ = pos;
}

void Impulse::Particle::velocity(Vector3f vel) {
    velocity_ = vel;
}

void Impulse::Particle::acceleration(Vector3f accel) {
    acceleration_ = accel;
}

Color Impulse::Particle::color() {
    return color_;
}

void Impulse::Particle::color(Color Col) {
    color_ = Col;
}
